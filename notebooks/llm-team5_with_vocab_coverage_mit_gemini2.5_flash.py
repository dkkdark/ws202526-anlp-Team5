...
def _(
    ...
    #using the Vocab comparision along with gemini-2.5-flash model
    ...
    def load_cambridge_vocab(path: str) -> set:
        """
        Load a vocab file in Cambridge A2/B1 style and return a set of headwords.
        Skips example lines and meta-text. Splits multi-form headwords.
        """
        vocab = set()
    ...
    # -------------------------------------------------------------------
    # Public API with same return type as your simple function
    # -------------------------------------------------------------------
...
def _(Path, json, simplify_sentence):
    in_path = Path(r"/__modal/volumes/vo-bu2NacTyYmRZLvaHb9L8eW/tsar2025_reduced_test.jsonl")

    out_path = Path(r"/__modal/volumes/vo-bu2NacTyYmRZLvaHb9L8eW/submission/team_test/results_llm_gemini_2.5_flash_with_vocab_coverage_reduced_gold.jsonl")
...
def _():
    ...
    # ---------------- Config ----------------
    GOLD_FILE = "/__modal/volumes/vo-bu2NacTyYmRZLvaHb9L8eW/tsar2025_reduced_test.jsonl"   # gold file next to this script
    SUBMISSIONS_DIR = "/__modal/volumes/vo-bu2NacTyYmRZLvaHb9L8eW/submission"     # folder with team subfolders
    ...
    df.to_excel("results_gemini_2.5_flash.xlsx", index=False)
    print("\nSaved: results.xlsx")
    return df, json
...
@app.cell
def _(df):
    df.to_csv("results_gemini_2.5_flash.csv", index=False)
    return
