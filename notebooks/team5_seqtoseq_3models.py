...
def _(
    ...
    if train_button.value:
        ...
        **Final Eval Loss**: {eval_result['eval_loss']:.4f}
        ...
        """)
    return device_t5, model_base, trainer
...
@app.cell
def _(mo):
    mo.image(src="/__modal/volumes/vo-KdoTRbHGUpf2m21hP3ZZ22/image.png")
    return
...
def _(AutoModelForSeq2SeqLM, AutoTokenizer, torch):
    if torch.cuda.is_available():
        device_base = torch.device("cuda")
    elif torch.backends.mps.is_available():
        device_base = torch.device("mps")
    else:
        device_base = torch.device("cpu")

    checkpoint_base = "google/flan-t5-base"
    ...
    def simplify_with_t5_base(text, model_base, tokenizer_base, device_base, max_length=128):
        """Simplify text using T5-large (non-finetuned)"""
        model_base.eval()

        input_text = "Simplify: " + text

        input_ids = tokenizer_base(
            input_text,
            max_length=max_length,
            truncation=True,
            return_tensors="pt",
        ).input_ids.to(device_base)
    ...
    return device_base, model_base, simplify_with_t5_base, tokenizer_base
...
def _(
    ...
    json,
    model_base,
    simplify_with_t5_base,
    tokenizer_base,
):


    in_path = Path("tsar2025_test.jsonl")
    out_path = Path("results_flant5-base.jsonl")
    ...
    with in_path.open("r", encoding="utf-8") as f:
        for line_no, line in enumerate(f, start=1):
            ...
            try:
                simplified = simplify_with_t5_base(original, model_base, tokenizer_base, device_base)
            except Exception as e:
                simplified = original
                print(f"[warn] line={line_no} text_id={text_id} level={level}: {e}")
...
def _(mo):
    ...
    - Target B1 (2) but predicted C1 (4) → error = 2
    These raise RMSE quickly.
...
def _(mo):
    mo.md(r"""
    ## Coverage (Validity)
    - All three systems have **200 outputs** and **100% coverage**.
    - This means the evaluation is complete and reliable.

    ## CEFR Targeting (Difficulty Control)

    ### FLAN‑T5‑Large (results_base_seqtoseq.jsonl)
    - **weighted_f1 = 0.4451**
    - **adj_accuracy = 0.910**
    - **rmse = 0.9274**

    **Interpretation:** Strong CEFR control among the three.

    ### FLAN‑T5‑Base (results_flant5-base.jsonl)
    - **weighted_f1 = 0.4100**
    - **adj_accuracy = 0.865**
    - **rmse = 1.0296**

    **Interpretation:** Middle performance on CEFR control.

    ### Fine‑tuned T5‑Base (results_finetuned_seqtoseq.jsonl)
    - **weighted_f1 = 0.3159**
    - **adj_accuracy = 0.735**
    - **rmse = 1.3115**

    **Interpretation:** Weakest CEFR control, often off by one or more levels.
    ...
    **Interpretation:** More aggressive simplification; lower similarity to original/reference.

    ### FLAN‑T5‑Base
    - **meaningbert-orig = 0.5517**
    - **bertscore-orig = 0.8780**
    - **meaningbert-ref = 0.5126**
    - **bertscore-ref = 0.8719**

    **Interpretation:** Most aggressive simplification; lowest semantic similarity.
    ...
    **Interpretation:** Most faithful to original meaning; simplification is mild.

    ## Trade‑off Summary

    - **FLAN‑T5‑Large**: best CEFR targeting overall, moderate meaning preservation.
    - **FLAN‑T5‑Base**: middle CEFR targeting, lowest semantic similarity (strongest simplification).
    - **Fine‑tuned T5‑Base**: best meaning preservation, weakest CEFR targeting.
    """)
    return
